...
export const CloudflareQuotaManager: React.FC<CloudflareQuotaManagerProps> = ({
  ...
  const loadData = async () => {
    try {
      ...
      setEstimate(data);
      if (data.config) {
        setHeartbeatInterval(data.config.heartbeatIntervalSeconds || 60);
        setRetentionDays(data.config.historyRetentionDays || 30);
        setClientPollInterval(data.config.clientPollIntervalSeconds || 30);
        setEcoMode(data.config.ecoMode ?? true);
        setAutoPrune(data.config.autoPruneExpiredHistory ?? true);
        setEdgeCacheAge(data.config.edgeCacheMaxAge || 30);
      }
    } catch (err: any) {
  ...
  return (
    <div className="space-y-6">
      {/* 1. Header Banner */}
      <div className="bg-gradient-to-r from-orange-500/10 via-amber-500/5 to-transparent p-6 rounded-2xl border border-orange-200 dark:border-orange-950/60 shadow-xs relative overflow-hidden">
