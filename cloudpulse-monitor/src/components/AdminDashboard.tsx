...
export const AdminDashboard: React.FC<AdminDashboardProps> = ({
  ...
  const [activeTab, setActiveTab] = useState<
    'nodes' | 'services' | 'incidents' | 'database' | 'deploy' | 'security' | 'webhooks' | 'sla' | 'audit' | 'public' | 'cloudflare'
  >('nodes');
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.success && res.token) {
        ...
        onShowToast('error', '密码错误', res.error || '默认密码为 admin123');
      }
    } catch (err: any) {
      ...
      <div id="admin-login-screen" className="max-w-md mx-auto my-12">
        <div className="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-xl p-8 space-y-6">
          ...
          <form onSubmit={handleLogin} className="space-y-4">
            ...
            <div className="p-3 bg-sky-50 dark:bg-sky-950/40 rounded-xl border border-sky-100 dark:border-sky-900/60 text-xs text-sky-800 dark:text-sky-300 flex items-center gap-2">
              <ShieldCheck className="w-4 h-4 shrink-0" />
              <span>
                系统初始默认管理密码: <strong className="font-mono">admin123</strong>
              </span>
            </div>
    ...
    <div id="admin-dashboard-container" className="space-y-6">
      ...
      <div className="flex items-center gap-2 border-b border-slate-200 dark:border-slate-800 pb-2 overflow-x-auto">
        ...
        >
          ...
          <span>服务器与探针管理 ({nodes.length})</span>
        </button>
        ...
        >
          ...
          <span>微服务拨测拓扑 ({services.length})</span>
        </button>
        ...
        >
          ...
          <span>故障通报与事件 ({incidents.length})</span>
        </button>
