...
export default function App() {
  const [activeTab, setActiveTab] = useState<'overview' | 'telegram' | 'incidents' | 'admin'>('overview');
  ...
  const handleTabChange = useCallback(
    (nextTab: 'overview' | 'telegram' | 'incidents' | 'admin') => {
  ...
  const loadData = useCallback(async (quiet = false) => {
    ...
    try {
      ...
      if (nodeRes) setNodes(nodeRes);
      if (incRes) setIncidents(incRes);
      if (historyRes) setMetricsHistory(historyRes);
      if (tgConfigRes) setTelegramConfig(tgConfigRes);
      if (tgLogsRes) setTelegramLogs(tgLogsRes);
    } catch (err: any) {
  ...
  // Fast Telegram alert trigger for a service
...
<i>消息由管理员手动触发推送验证。</i>`;
  ...
  // Fast Telegram alert trigger for a node
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8">
        ...
        {isInitialLoading || isTabSwitching || isManualRefreshing ? (
          ...
          <>
            ...
            {/* Tab 2: Telegram Push & Alerts Center (Push Only) */}
            {activeTab === 'telegram' && (
              <TelegramBotHub
